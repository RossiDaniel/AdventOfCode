...
op :: [Int] -> [Int] -> Int -> [Int]
op inp xs i  | head istr == 1 = op inp (somma xs i istr) (i+4)
             | head istr == 2 = op inp (prodotto xs i istr) (i+4)
             | head istr == 3 = op (drop 1 inp) (inputistr xs i inp) (i+2)
             | head istr == 4 = (outputistr xs i istr):(op inp xs (i+2))
             | head istr == 5 = if (mode xs (i+1) (istr!!2)) /= 0 then op inp xs (mode xs (i+2) (istr!!3))  else op inp xs (i+3) 
             | head istr == 6 = if (mode xs (i+1) (istr!!2)) == 0 then op inp xs (mode xs (i+2) (istr!!3))  else op inp xs (i+3) 
             | head istr == 7 = if (mode xs (i+1) (istr!!2)) < (mode xs (i+2) (istr!!3)) then op inp (substitute xs (xs!!(i+3)) 1) (i+4)  else op inp (substitute xs (xs!!(i+3)) 0) (i+4) 
             | head istr == 8 = if (mode xs (i+1) (istr!!2)) == (mode xs (i+2) (istr!!3)) then op inp (substitute xs (xs!!(i+3)) 1) (i+4)  else op inp (substitute xs (xs!!(i+3)) 0) (i+4) 
             | otherwise    = []
             where
                istr = int2istr (xs!!i)
